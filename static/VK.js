(function (w) {
  ...
  function envLoaded () {
    ...
    for (let i = 0, len = onEnvLoad.length; i < len; i++) {
  ...
  function applyMethod (strData, self, origin) {
    getEnv(function (env) {
      const data = env.json.parse(strData)
      const method = data[0]
      ...
      if (method) {
        if (
          ...
          throw new Error('Insecure method call.')
        }
        ...
        setTimeout(function () {
          if (!self.methods[method]) {
            throw new Error('fastXDM: Method ' + method + ' is undefined')
          }
  ...
  w.fastXDM = {
        ...
        getEnv(function (env) {
          for (const method in this.methods) {
            if (this.methods[method] instanceof w.fastXDM.Server) {
              const proxyFromServer = this
              const proxyToServer = this.methods[method]
    ...
    Client (methods, options) {
      ...
      if (window.name.indexOf('fXD') === 0) {
        ...
        throw new Error('Wrong window.name property.')
      }
    ...
    onMessage (e) {
      ...
      const key = data.substr(0, 5)
      if (handlers[key]) {
        const self = handlers[key][1]
  ...
  w.fastXDM.Client.prototype.callMethod = w.fastXDM.Server.prototype.callMethod = function () {
    ...
    waitFor(this, 'caller', function () {
      w.fastXDM.on(this.id, function () {
        getEnv(function (env) {
          let xdm = this
          const proxyToServer = w.fastXDM.getProxyToServer(this.key, method)
  ...
  if (w.postMessage) {
    ...
    env.send = function (xdm, strData) {
      const win = (xdm.frame ? xdm.frame.contentWindow : xdm.caller)
      if (win) {
        try {
...
VK.init = function (success, failure, ver) {
  ...
  if (!VK._inited) {
    ...
    VK.initXDConn()
  } else if (VK.isFunc(success)) { success() }
  const activeEvents = ['mousemove', 'keydown']
  for (let i = 0; i < 0; i++) {
...
VK.loadParams = function (q) {
  if (typeof q === 'Object') { VK.params = q } else {
    const tmp = q.substr(q.indexOf('?') + 1).split('&')
    let i = tmp.length
    while (i--) {
      const v = tmp[i].split('=')
...
(function () {
  try {
    ...
    const domain = script.src.match(/^https?:\/\/((?:\w+\.)*vk.com)/)[1]
    if (domain) {
...
if (!VK.Widgets) {
  VK.Widgets = (function () {
    const obj = {}
    const widgetlist = ['Comments', 'CommentsBrowse', 'Recommended', 'Post', 'Like', 'Poll', 'Group', 'Auth', 'Subscribe', 'ContactUs', 'Ads', 'AllowMessagesFromCommunity', 'CommunityMessages']
    VK.xdConnectionCallbacks = []
    let i = widgetlist.length
    while (i--) {
      (function (f) {
        obj[f] = function () {
          ...
          if (!VK._openApiAttached) {
            VK.callMethod('_getAppInfo', function (data) {
              const baseDomain = VK._domain && VK._domain.base && VK._domain.base.match(/^(\w+\.)*vk.com$/) ? VK._domain.base : 'https://vk.com'
              VK._apiId = data[0]
...
/* Obsolete methods */
VK.External = {
  showPaymentBox (a) { VK.callMethod('showPaymentBox', a) },
  showSettingsBox (a) { VK.callMethod('showSettingsBox', a) },
  showInstallBox () { VK.callMethod('showInstallBox') },
  showInviteBox () { VK.callMethod('showInviteBox') },
  resizeWindow (b, a) { VK.callMethod('resizeWindow', b, a) },
  scrollWindow (b, a) { VK.callMethod('scrollWindow', b, a) },
  setLocation (a, b) { VK.callMethod('setLocation', a, b) },
  setTitle (a) { VK.callMethod('setTitle', a) },
  showProfilePhotoBox (a) { VK.callMethod('showProfilePhotoBox', a) },
  showMerchantPaymentBox (a) { VK.callMethod('showMerchantPaymentBox', a) }
}
